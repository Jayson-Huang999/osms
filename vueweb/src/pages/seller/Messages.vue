<template>
  <div class="Messages">
    ...
    <div v-if="noReplyMsgList.length==0 && curIndex==0" class="empty">
      <p>暂无任何数据...</p>
    </div>
    <div v-if="repliedMsgList.length==0 && curIndex==1" class="empty">
      <p>暂无任何数据...</p>
    </div>
...
<script>
import {getNoReplyMsg, getRepliedMsg, reply} from '../../api/seller';
...
export default {
  ...
  methods: {
    ...
    getNoReplyMsg() {
      ...
      const res = getNoReplyMsg(this.sellerId);
      res.then((msgs) => {
        if (msgs == null) {
          this.noReplyMsgList = [];
        } else {
    ...
    getRepliedMsg() {
      const res = getRepliedMsg(this.sellerId);
      res.then((msgs) => {
        if (msgs == null) {
          this.repliedMsgList = [];
        } else {
