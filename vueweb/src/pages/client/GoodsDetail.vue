        ...
        <div class="content">
            <div class="goodsInfo">
                ...
                <div class="infoRight">
                    ...
                    <div class="infoBox">
                        <span>数量：</span>
                        <NumberInput v-model="num" :min="1" :max="this.activity?1:temStockNum"/>
                        <a v-if="hasToken" href="#" @click="store" style="position: relative;left: 60px;top: 25px;">
                            <img :src="imgUrl" height="50" width="50"></a>
                    </div>
                    <button class="buyBtn" @click="confirmOrder">立即购买</button>
    ...
    export default {
        ...
        methods: {
            ...
            confirmOrder(route) {
                ...
                this.specs.map((item, index) => {
                    if (item.id == this.temSpecId) {
                        this.setOrdersToConfirm(JSON.stringify(
                            [{
                                specification_id: this.temSpecId,
                                price: item.price,
            ...
            //     quantity:this.num,
            //     preferentialprice:0,
            //     totalprice:this.num*this.goodsPrice,
            //     createtime:null
            //   }]);
            //   res
            //   .then(()=>{
        ...
        mounted() {
            ...
            this.timer = setInterval(() => {
                ...
                getProductStock(this.id).then((data)=>{
                    this.specs.map((item,index)=>{
                        item.stock=data[item.id.toString()].stock;
                    });
                    })
                    .catch((e)=>{
                        alert(e);
