...
<script>
    ...
    export default {
        ...
        //     ...mapGetters(['checkNum', 'totalPrice', 'checkGoods'])
        // },
          methods: {
            values(data){
            ...
            generateOrders() {
                ...
                for (let i = 0; i < this.checkGoods.length; i++) {
                    ordersList.push({
                        ...
                        specificationId:this.checkGoods[i].specification_id,
                        userId:this.clientId,
                        status:1,
                        quantity:this.checkGoods[i].num,
                    ...
                    stockList.push({
                        ...
                        num:this.checkGoods[i].num
                    })
                    if(this.checkGoods[i].activity_id>=1){
                        remainList.push({
                            activity_id:this.checkGoods[i].activity_id,
                            num:1
                        })
                    }
                }
                const res = generateOrders(ordersList);
                res
                    .then((data)=>{
                ...
                updateSpecificationStock(stockList).then(()=>{})
                    .catch((e)=>{
                        alert(e);
                    })
                updateActivityRemain(remainList).then(()=>{})
                    .catch((e)=>{
                        alert(e);
                    })
            },
